...
from .exceptions import ProxmoxError, ConfigurationError
...
def get_proxmox_client() -> ProxmoxAPI:
    ...
    try:
        if token_id and token_secret:
            log_info_blue(logger, f"Attempting Proxmox API connection to {host}:{port} using API token.")
            ...
            log_info_blue(logger, f"Using Proxmox token user string: '{api_user_string}'.")
            ...
            log_info_blue(logger, f"Attempting Proxmox API connection to {host}:{port} using password for user '{user_full}'.")
        ...
        log_info_blue(logger, "ProxmoxAPI client instantiated. Verifying connection with version check...")
        version_info = temp_client.version.get() 
        log_info_green(logger, f"Successfully connected to Proxmox API on {host}:{port}. Version: {version_info.get('version')}")
        ...
        if isinstance(error_content, dict) and 'data' in error_content and error_content['data'] is None:
        ...
        raise ProxmoxError(f"Proxmox API call error for {host}:{port}. Error: {error_content}") from e
    except Exception as e: 
        # This is an error during ProxmoxAPI instantiation itself
        log_info_yellow(logger, f"Failed to initialize or connect with ProxmoxAPI client: {str(e)}")
        raise ProxmoxError(f"Failed to establish connection with Proxmox API at {host}:{port}. Root cause: {str(e)}") from e

    if not ssl_verify and _proxmox_client: 
        import urllib3 
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        log_info_yellow(logger, "Warning: SSL verification for Proxmox API is disabled. InsecureRequestWarning suppressed.")

    if not _proxmox_client:
        # This safeguard should ideally not be hit if exceptions are handled correctly above
        raise ProxmoxError("Proxmox client could not be initialized for an unknown reason after attempting connection.")
    return _proxmox_client
...
def get_proxmox_cluster_nodes() -> List[str]:
    ...
    try:
        ...
        node_names = sorted([node_info['node'] for node_info in nodes_data if 'node' in node_info])
        ...
        logger.error(f"  Failed to get Proxmox nodes: {e.content if hasattr(e, 'content') and e.content else str(e)}")
...
def get_vms_on_node(node_name: str) -> List[int]:
    ...
    try:
        ...
        log_info_green(logger, f"    Discovered VMs on node '{node_name}': {', '.join(map(str, vmids))}")
        return vmids
    except ResourceException as e:
        error_detail = e.content if hasattr(e, 'content') and e.content else str(e)
        log_info_yellow(logger, f"    Warning: Failed to get VMs for node '{node_name}'. Node may be unreachable or API error occurred. Detail: {error_detail}")
...
def get_vm_config(node_name: str, vmid: int) -> Dict[str, Any]:
    ...
    log_info_blue(logger, f"    Fetching config for VM '{vmid}' on node '{node_name}'...")
    client = get_proxmox_client()
    try:
        config_data = client.nodes(node_name).qemu(vmid).config.get()
        return config_data
    except ResourceException as e:
        logger.error(f"      Failed to get config for VM {vmid} on {node_name}: {e.content if hasattr(e, 'content') and e.content else str(e)}")
        raise ProxmoxError(f"Could not fetch config for VM {vmid} on {node_name}.") from e
...
def get_vm_status(node_name: str, vmid: int) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"    Failed to get status for VM {vmid} on {node_name}: {e.content if hasattr(e, 'content') and e.content else str(e)}")
        raise ProxmoxError(f"Could not fetch status for VM {vmid} on {node_name}.") from e
...
def control_vm(node_name: str, vmid: int, action: str) -> str:
    ...
    try:
        task_id = getattr(client.nodes(node_name).qemu(vmid).status, pve_api_action).post()
        log_info_green(logger, f"    Successfully initiated {pve_api_action} for VM {vmid} on {node_name}. Task ID: {task_id}")
        return task_id
    except ResourceException as e:
        logger.error(f"    Failed to {action} VM {vmid} on {node_name}: {e.content if hasattr(e, 'content') and e.content else str(e)}")
        raise ProxmoxError(f"Could not {action} VM {vmid} on {node_name}.") from e
    except AttributeError: 
        logger.error(f"    Invalid Proxmox API action '{pve_api_action}' derived from '{action}'.")
...
def set_vm_network_config(
    ...
    """Sets the network configuration for a VM using cloud-init parameters."""
    log_info_blue(logger, f"    Configuring network for VM {vmid} on {node_name}...")
    client = get_proxmox_client()
    ...
    try:
        ...
        log_info_green(logger, f"      Successfully set network configuration for VM {vmid}. Result: {result}")
        return str(result) 
    except ResourceException as e:
        logger.error(f"      Failed to set network config for VM {vmid}: {e.content if hasattr(e, 'content') and e.content else str(e)}")
