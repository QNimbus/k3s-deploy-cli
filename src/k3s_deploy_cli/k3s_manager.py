...
from .exceptions import (
    NodeDiscoveryError, ConfigurationError, ProxmoxError, K3sDeployError
)
from .logging_utils import logger, log_info_green, log_info_blue, log_info_yellow
...
class K3sDeploymentManager:
    ...
    def _parse_vm_location(self, location_str: str) -> Tuple[str, int]:
        ...
        if ':' not in location_str:
            raise ValueError(f"Invalid VM location format: '{location_str}'. Expected 'node:vmid'.")
        parts = location_str.split(':', 1)
        try:
            return parts[0], int(parts[1])
        except ValueError:
            raise ValueError(f"Invalid VMID in location: '{location_str}'. VMID must be an integer.")
    ...
    def _populate_node_lists(self) -> None:
        ...
        log_info_blue(logger, "  Sorted discovered node lists for deterministic processing.")
        ...
        if self.servers:
            ...
            if self.servers_no_first:
                log_info_blue(logger, f"  Remaining servers (excluding master): {', '.join(map(str, self.servers_no_first))}")
            else:
        ...
        if self.servers:
            log_info_green(logger, f"Discovered server entries: {', '.join(map(str, self.servers))}")
        if self.agents:
            log_info_green(logger, f"Discovered agent entries: {', '.join(map(str, self.agents))}")
        if self.storage:
            log_info_green(logger, f"Discovered storage entries: {', '.join(map(str, self.storage))}")
    ...
    def load_nodes_from_config_file(self) -> bool:
        ...
        log_info_blue(logger, f"Loading node information from {app_config.NODE_CONFIG_FILE}...")
        try:
            with open(app_config.NODE_CONFIG_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
        except (json.JSONDecodeError, IOError) as e:
            raise ConfigurationError(f"Failed to load or parse {app_config.NODE_CONFIG_FILE}: {e}")
        ...
        for p_node_info in data.get("nodes", []):
            ...
            for vm_info in p_node_info.get("vms", []):
                ...
                if role == app_config.SERVER_TAG.upper() or role == "SERVER": # "SERVER" for simpler JSON
        ...
        self.all_nodes = list(set(self.servers + self.agents + self.storage)) # Unique list
    ...
    def discover_nodes_by_tags(self) -> None:
        ...
        if not proxmox_host_nodes:
            raise NodeDiscoveryError("No Proxmox host nodes found. Cannot discover VMs by tags.")
        ...
        for p_node_name in proxmox_host_nodes:
            ...
            for vmid in vmids_on_node:
                ...
                tags_str: str = vm_config.get("tags", "") # Tags are semicolon-separated
                vm_tags: List[str] = [tag.strip() for tag in tags_str.split(';') if tag.strip()]

                if not vm_tags:
                    continue

                vm_identifier = VMIdentifier(proxmox_node=p_node_name, vmid=vmid, name=vm_name, tags=vm_tags, config=vm_config)
                ...
                log_info_green(logger, f"      Found VM: {vm_identifier} with tags: {', '.join(vm_tags)}")
                self.all_nodes.append(vm_identifier)
                if is_server:
                    self.servers.append(vm_identifier)
                    log_info_green(logger, f"      Discovered K3s server: {vm_identifier}")
                if is_agent:
                    self.agents.append(vm_identifier)
                    log_info_green(logger, f"      Discovered K3s agent: {vm_identifier}")
                if is_storage:
                    self.storage.append(vm_identifier)
                    log_info_green(logger, f"      Discovered K3s storage: {vm_identifier}")
    ...
    def ensure_nodes_are_discovered(self, discover_if_empty: bool = True) -> None:
        ...
        if self.load_nodes_from_config_file():
            if self.all_nodes:
                ...
                log_info_yellow(logger, f"Config file {app_config.NODE_CONFIG_FILE} loaded but no K3s nodes defined.")
        
        if discover_if_empty:
            log_info_blue(logger, f"{app_config.NODE_CONFIG_FILE} not found or empty. Attempting tag-based discovery...")
    ...
    def check_k3s_version(self, ask_update: bool = False) -> None:
        ...
        try:
            ...
            if not latest_tag_name:
                log_info_yellow(logger, "Warning: Could not determine latest K3s version from GitHub API response.")
                return

            log_info_blue(logger, f"  Current K3s version configured: {self.k3s_version}")
            log_info_blue(logger, f"  Latest K3s release on GitHub: {latest_tag_name}")

            if self.k3s_version != latest_tag_name:
                log_info_yellow(logger, f"  Info: Current K3s version {self.k3s_version} differs from latest release {latest_tag_name}.")
                if ask_update:
                    answer = input(f"Do you want to use the latest K3s version ({latest_tag_name})? (y/n): ").strip().lower()
                    if answer == 'y':
                        ...
                        log_info_green(logger, f"Updated to use latest K3s version: {self.k3s_version}")
                    else:
                        log_info_blue(logger, f"Continuing with current version: {self.k3s_version}")
            else:
            ...
            log_info_yellow(logger, "Warning: Failed to parse K3s version from GitHub API response.")
    ...
    def perform_vm_action(self, action: str) -> None:
        ...
        log_info_blue(logger, f"Performing Proxmox VM action '{action}' on discovered K3s VMs...")

        if not self.all_nodes:
            log_info_yellow(logger, f"Warning: No VM locations discovered/loaded. Skipping Proxmox VM action '{action}'.")
            return
        ...
        for vm in self.all_nodes:
            ...
            try:
                ...
                logger.error(f"    Could not get status for {vm}. Skipping action. Error: {e}")
                continue
            ...
            if action == "start":
                if current_status == "running":
                    ...
                    log_info_blue(logger, f"    VM {vm} is currently '{current_status}'. Attempting to start...")
                ...
                if current_status == "stopped":
                    ...
                    log_info_blue(logger, f"    VM {vm} is currently '{current_status}'. Attempting graceful shutdown...")
                    proxmox_api.control_vm(vm.proxmox_node, vm.vmid, "stop") # 'stop' maps to 'shutdown' in control_vm
            elif action == "restart":
                if current_status != "running":
                    log_info_yellow(logger, f"    VM {vm} is not running (status: '{current_status}'). Starting it instead of rebooting.")
                    # Alternative: skip or error. Bash script started it.
                    proxmox_api.control_vm(vm.proxmox_node, vm.vmid, "start")
                else:
                    log_info_blue(logger, f"    VM {vm} is currently 'running'. Attempting to reboot...")
    ...
    def configure_ips(self) -> None:
        """
        Assigns static IPs from the defined range to VMs with K3s role tags.
        """
        self.ensure_nodes_are_discovered()
        log_info_blue(logger, "Starting IP configuration for VMs with K3s role tags")

        if not self.all_nodes:
            log_info_yellow(logger, "Warning: No VM locations discovered/loaded. Skipping IP configuration.")
            return

        try:
            ip_range_start_obj = ipaddress.IPv4Address(app_config.K3S_NODE_IP_RANGE_START)
        ...
        for vm in sorted_nodes:
            ...
            if not vm.config or f"ipconfig{app_config.K3S_NODE_IPCONFIG_INDEX}" not in vm.config:
                try:
                    vm.config = proxmox_api.get_vm_config(vm.proxmox_node, vm.vmid)
                    vm.name = vm.config.get("name", vm.name) # Update name if available
                except ProxmoxError as e:
                    logger.error(f"    Could not get config for {vm} to check current IP. Skipping. Error: {e}")
                    continue
            ...
            if next_ip_to_assign_int > ip_range_end_int:
                ...
                logger.error(f"      Cannot configure {vm}. Please expand the range or check assignments.")
            ...
            target_nameserver_value = " ".join(app_config.K3S_NODE_DNS_SERVERS) # Space-separated for pvesh
            ...
            if current_ipconfig_value != target_ipconfig_value:
                needs_update = True
            if target_nameserver_value and current_nameserver_value != target_nameserver_value:
                needs_update = True
            if target_searchdomain_value and current_searchdomain_value != target_searchdomain_value:
                needs_update = True
            ...
            if target_searchdomain_value:
                log_info_blue(logger, f"      Adding search domain: {target_searchdomain_value}")
            ...
            try:
                ...
                log_info_yellow(
                    logger,
                    f"      Note: {vm_display_name} may need a reboot for cloud-init to apply these network settings."
                )
            except ProxmoxError as e: # Already logged in proxmox_api
                logger.error(f"      Skipping IP configuration for {vm_display_name} due to previous error.")
        ...
        log_info_green(logger, "IP configuration process completed.")
    ...
    def provision_k3s_cluster(self) -> None:
        ...
        if not self.server_master:
            logger.error("Error: No K3s server master node defined or discovered for provisioning. Aborting K3s setup.")
            raise K3sDeployError("Cannot provision K3s: No master server found.")

        log_info_blue(logger, "IMPORTANT: This section assumes SERVERS, AGENTS, and STORAGE lists contain "
                              "nodes that are reachable and correctly configured (e.g., via IP addresses).")
        log_info_blue(logger, f"Current SERVER_MASTER (first master target): {self.server_master}")
        if self.servers_no_first:
            log_info_blue(logger, f"Other server nodes: {', '.join(map(str, self.servers_no_first))}")
        if self.agents:
        ...
        # A further step might be needed to *discover* these applied IPs before provisioning.
        log_info_yellow(logger, "K3s provisioning logic is a placeholder and needs to be implemented.")
        # --- End K3s Provisioning Logic ---

        log_info_green(logger, "K3s provisioning placeholder finished.")